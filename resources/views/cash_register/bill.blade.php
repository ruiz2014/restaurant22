            ...
            {!! $errors->first('payMethod', '<div class="invalid-feedback d-block" role="alert"><strong>:message</strong></div>') !!}
                ...
                @foreach($payment_methods as $pay)    
                    <li class="list-group-item d-flex">
                        <div class="w-50">
                            <input class="form-check-input me-1" name="payMethod[]" type="checkbox" value="{{ $pay->id }}" aria-label="..." form="prue">
                            {{ $pay->name }}
                        </div>
                        
                        <input type="number" class="form-control-2" name="payMethodVal[]" id="payMethod_{{ $pay->id }}" placeholder="Monto S/."  style="width:50%;display:none" form="prue">
                    </li>
                @endforeach    
                </ul>
            </div>
    ...
    window.addEventListener("load", function(){
        ...
        payMethod.onclick = function(ev){
            ...
            if(ev.target.checked){
                ...
                document.getElementById(`payMethod_${ev.target.value}`).style.display="none"
