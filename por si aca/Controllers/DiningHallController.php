...
class DiningHallController extends Controller
{
    ...
    public function sendToKitchen(Request $req){
        ...
        if($check){
            $sendOrders = Product::select("products.name", "products.price", "to.table_id", "to.id", "to.status", "to.amount", "to.note", "to.created_at")->join("temp_orders as to", "to.order_id", "=", "products.id")
                                ->where('to.code', $check)
                                ->where('to.status', 1)
                                ->get();
